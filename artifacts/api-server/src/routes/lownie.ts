...
import {
  ...
  CreateLownieApplicationBody,
  DownloadLownieTicketParams,
  GetLownieConfigResponse,
  InitializeLowniePaymentBody,
  VerifyLowniePaymentParams,
...
router.get("/lownie/tickets/:applicationId/download", async (req, res) => {
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="lownie-${application.lownieId}.pdf"`,
  );
  ...
  document.fontSize(10).fillColor("#5c6b68").text(
    "This Lownie ticket confirms that your application was received. It is not a CRB report or a promise of loan approval. Loan decisions are made by the relevant financial service provider.",
    { width: 480 },
  );
  document.end();
  return res;
