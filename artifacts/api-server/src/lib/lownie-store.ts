...
const memoryApplications = new Map<string, LownieApplication>();
const memoryVerificationSessions = new Map<string, VerificationSession>();
let memoryConfig: LownieConfig | undefined;
let mongoClient: MongoClient | undefined;
let mongoCollection: Collection<LownieApplication> | undefined;
...
async function getMongoCollection() {
  ...
  try {
    ...
    mongoCollection = database.collection<LownieApplication>("applications");
...
export async function getLownieConfig(): Promise<LownieConfig> {
  ...
  const settings = collection.db.collection<LownieConfig & { _id: string }>(
    "settings",
  );
  const existing = await settings.findOne({ _id: "lownie" });
  if (existing) return { feeKes: existing.feeKes, currency: "KES" };

  const created = { _id: "lownie", feeKes: configuredFee, currency: "KES" as const };
...
export async function createApplication(
  input: Omit<LownieApplication, "applicationId" | "createdAt" | "paid" | "status">,
) {
  const application: LownieApplication = {
    ...input,
