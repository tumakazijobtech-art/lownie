...
import {
  getDownloadLownieTicketQueryKey,
  getVerifyLowniePaymentQueryKey,
  useConfirmPhoneVerification,
  useCreateLownieApplication,
  useDownloadLownieTicket,
  useGetLownieConfig,
  useInitializeLowniePayment,
  useStartPhoneVerification,
  useVerifyLowniePayment,
  type LownieApplication,
...
function Home() {
  const configQuery = useGetLownieConfig();
  const startVerification = useStartPhoneVerification();
  const confirmVerification = useConfirmPhoneVerification();
  const createApplication = useCreateLownieApplication();
  const initializePayment = useInitializeLowniePayment();
  ...
  const [application, setApplication] = useState<LownieApplication | null>(null);
  ...
  const paymentQuery = useVerifyLowniePayment(paymentReference, {
    query: {
      enabled: Boolean(paymentReference),
      queryKey: getVerifyLowniePaymentQueryKey(paymentReference),
      retry: false,
    },
  });
  const ticketQuery = useDownloadLownieTicket(application?.applicationId ?? '', {
    query: {
      enabled: false,
      queryKey: getDownloadLownieTicketQueryKey(application?.applicationId ?? ''),
      retry: false,
    },
  });
  ...
  const handleDownloadTicket = async () => {
    ...
    link.href = url;
    link.download = `lownie-${application?.lownieId ?? 'ticket'}.pdf`;
    ...
    <main className="min-h-[100dvh] bg-[#f5f8f9] text-[#173945]">
      <div className="mx-auto min-h-[100dvh] max-w-5xl px-5 py-6 sm:px-8 sm:py-8">
        <header className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3" data-testid="brand-lownie">
            <div className="grid h-10 w-10 place-items-center rounded-xl bg-[#174b5b] text-white">
              <span className="text-lg font-bold">N</span>
            </div>
            <div>
              <p className="text-base font-bold tracking-tight">LOWNIE</p>
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-[.75fr_1.25fr] lg:gap-14 lg:pt-8">
          <aside className="lg:sticky lg:top-8 lg:self-start">
            ...
            <p className="mt-6 max-w-sm text-sm leading-6 text-[#63747a]">
              Lownie helps you prepare your request, understand your options, and keep a record of your application.
            </p>
            ...
            {step === 2 ? (
              <Panel>
                ...
                <label className="mt-7 flex cursor-pointer gap-3 rounded-lg border border-[#dbe3e5] bg-[#f8fafb] p-4 text-sm leading-6 text-[#536b72]">
                  ...
                  />
                  <span>I agree that Lownie may use this information to support my application and provide relevant guidance.</span>
                </label>
                <button
            ...
            {step === 3 ? (
              <Panel>
                <div className="mb-8">
                  ...
                  <p className="mt-2 max-w-md text-sm leading-6 text-[#63747a]">A small service fee covers application processing and your downloadable Lownie ticket.</p>
                </div>
                ...
                {!paymentStarted ? (
                  <div className="space-y-5">
                    ...
                    <div className="flex gap-3 rounded-lg border border-[#dbe3e5] bg-[#f8fafb] p-4 text-xs leading-5 text-[#63747a]">
                      ...
                      <p>Payments are securely processed by Paystack. Lownie does not see your card details.</p>
                    </div>
                    <button
            ...
            {step === 4 && application ? (
              <Panel className="overflow-hidden">
                <div className="relative -mx-5 -mt-5 mb-7 overflow-hidden bg-[#174b5b] px-5 py-8 text-white sm:-mx-8 sm:-mt-8 sm:px-8">
                  ...
                  <div className="relative flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="mt-2 text-sm leading-6 text-[#c6dcda]">Your application has been received. Keep this Lownie ID for reference.</p>
                    </div>
                  </div>
                </div>
                <div className="rounded-xl border-2 border-dashed border-[#bfd0d0] bg-[#f8fafb] p-5 text-center sm:p-7">
                  <p className="text-xs font-semibold uppercase tracking-[.16em] text-[#718087]">Lownie ID</p>
                  <p className="mt-2 font-mono text-3xl font-medium tracking-[.06em] text-[#174b5b]" data-testid="text-lownie-id">{application.lownieId}</p>
                ...
                <div className="mt-7 flex items-start gap-3 border-t border-[#e1e8e9] pt-5 text-xs leading-5 text-[#63747a]">
                  ...
                  <p>Save this ticket. Use your Lownie ID when contacting our support team.</p>
                </div>
              </Panel>
            ) : null}
          </section>
